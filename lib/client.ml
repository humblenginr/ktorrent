...
let init_peer client peer info_hash peer_id =
   match%lwt (Utils.compute_promise_with_timeout (Peer.connect peer ) 3.) with
   | `Done connected_peer -> 
       begin 
        match%lwt (Utils.compute_promise_with_timeout (Peer.complete_handshake connected_peer info_hash peer_id) 3.) with
          | `Done _ -> 
             begin
              match%lwt Utils.compute_promise_with_timeout (Peer.receive_bitfield connected_peer) 3. with
              | `Done bitfield -> 
                  begin
...
let init_peers client peers_list info_hash peer_id = 
  List.map ~f:(fun peer -> 
    (*try syntax is used to make sure that the inability to initialize to a single peer does not reject the whole promise *)
    try%lwt init_peer client peer info_hash peer_id with _ -> Lwt.return ()
    ) peers_list |> Lwt.all
